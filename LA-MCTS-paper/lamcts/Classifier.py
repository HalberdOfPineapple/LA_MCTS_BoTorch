...
class Classifier():
    ...
    def propose_rand_samples_sobol(self, nums_samples, path, lb, ub):
        ...
        # centers are the samples inside the node's region
        ratio_check, centers = self.get_sample_ratio_in_region(self.X, path)

        # no current samples located in the region should not happen
        # print("ratio check:", ratio_check, len(self.X) )
        # assert ratio_check > 0
        if ratio_check == 0 or len(centers) == 0:
            rand_cands = self.propose_rand_samples( nums_samples, lb, ub)
            print(f"[Classifier] No samples in the region, {len(rand_cands)} random samples are generated.")
            return rand_cands
        ...
        for center in centers:
            # center is a randomly selected list of samples in the current node
    ...
    def propose_samples_bo( self, nums_samples = 10, path = None, lb = None, ub = None, samples = None):
        ...
        n = min(nums_samples, X_ei.shape[0])
    ...
    def propose_samples_turbo(self, num_samples, path, func):
        ...
        num_init = 30
        X_init = self.propose_rand_samples_sobol(num_init, path, func.lb, func.ub)

        #get samples around the selected partition
        print("sampled ", len(X_init), " for the initialization")
        if num_samples <= num_init:
            proposed_X = X_init[:num_samples]
            fX = np.array([func(x) for x in proposed_X])
        else:
            turbo1 = Turbo1(
    ...
    def split_data(self):
        ...
        # print('-' * 50)
        # print("Trained SVM prediction on good samples:")
        # print(self.svm.predict(train_good_samples))

        # print("Trained SVM prediction on bad samples:")
        # print(self.svm.predict(train_bad_samples))
