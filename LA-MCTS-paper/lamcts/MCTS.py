...
class MCTS:
    ...
    def __init__(self, 
                 lb, ub, dims, ninits, func, 
                 Cp = 1, leaf_size = 20, kernel_type = "rbf", gamma_type = "auto",
                 solver_type="bo"):
        self.dims                    =  dims
        self.samples                 =  []
        self.nodes                   =  []
    ...
    def greedy_select(self):
        ...
        while curt_node.is_leaf() == False:
            ...
            # choice - the index of the child node with the highest x_bar
            choice = np.random.choice(np.argwhere(UCT == np.amax(UCT)).reshape(-1), 1)[0]

            # path - a list of tuples (node, choice) that we have traversed
    ...
    def select(self):
        ...
        while curt_node.is_leaf() == False:
            UCT = []
            for child_node in curt_node.kids:
                UCT.append(child_node.get_uct(self.Cp) )
    ...
    def search(self, iterations):
        for idx in range(self.sample_counter, iterations):
            ...
            if self.solver_type == 'bo':
                ...
                samples, values = leaf.propose_samples_turbo(iterations - len(self.samples), path, self.func )
            else:
                raise Exception("solver not implemented")

            print(f'[MCTS] search iteration {idx}: sampled {len(samples)} samples by {self.solver_type}')
            ...
            if len(self.samples) >= iterations:
                break
