...
class Levy:
    ...
    def __call__(self, x):
        ...
        result = self.levy(torch.Tensor(x.reshape(-1, self.dims))).numpy()[0]
        return result
...
class Ackley:
    def __init__(self, dims=10):
        ...
        self.ackley = BoAckley(
            dim=self.dims,
            bounds=[(-5., 10.) for _ in range(self.dims)])
    ...
    def __call__(self, x):
        ...
        result = float(self.ackley(torch.Tensor(x.reshape(-1, self.dims))).numpy()[0])
        # self.tracker.track( result )
